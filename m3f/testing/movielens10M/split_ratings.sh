...
# the above line is the original, the cut command does not work somehow
# the following works as first counting the line number,
# then delete the first space character from output, leaving only int
RATINGS_COUNT=`wc -l < ratings.dat | cut -c 2-`
...
sort -t"::"" -k 4,1n -k 2,2n ratings.dat > ratings.time
for i in 1 2 3 4 5
  do
